...
const projects: ProjectItem[] = [
  {
    ...
    rating: '4.9',
    icon: <BsBookmarkDashFill className=" text-blue-900 w-[16px] h-[19px] ml-auto" />,
    img: Agence
  },
  {
    name: 'CSB Klinik',
    ...
    <section className="w-full h-full relative flex-1 flex flex-col gap-10 md:gap-0 lg:p-4 lg:pt-0 grow ">
      ...
      <h1 className="name_title flex flex-col items-center gap-2">
        {arraySquare.map((group, groupIndex) => (
          <div key={`group-${groupIndex}`} className="flex justify-center w-full">
      ...
      {/* <div
        ...
        className={cn(
          'w-12 h-12 rounded-full absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 flex justify-center items-center glassomorph_effect cursor-pointer z-50 ',
          show && 'hide'
        )}
      >
...
const BottomSection = () => {
  return (
    <>
      <LeftBottom />
      <div className="w-full h-full glassomorph_effect_bis p-4">Veeppee current workplace</div>
    </>
  )
}
const LeftBottom = () => {
  return (
    <div className="glassomorph_effect_bis w-[70%]  xl:w-[65%] p-4 lg:h-full ">
      {/* TItle */}
      <h3 className="tracking-wider text-2xl font-bold  mb-2 xl:mb-8">Techno</h3>
      {/* Carousel */}
      <Swiper
        breakpoints={{
          0: {
